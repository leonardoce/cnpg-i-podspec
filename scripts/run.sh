...
# kind load docker-image --name=${current_context} cnpg-i-podspec:${VERSION:-latest}
...
load_image_registry cnpg-i-podspec:${VERSION:-latest}

# Patch the deployment to use the provided image
kubectl patch deployments.apps -n cnpg-system podspec -p \
  "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"cnpg-i-podspec\",\"image\":\"${registry_name}:5000/cnpg-i-podspec:${VERSION:-latest}\"}]}}}}"
